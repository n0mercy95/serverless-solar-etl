...
2. **IrradianceOutlierStrategy (Filtro Físico)**: Se analizan las desviaciones severas entre la predicción meteorológica (NWP) y la medición local (LMD). Si el ratio NWP/LMD cae fuera de un umbral físico aceptable [0.3, 3.0], se descartan como anomalías (outliers).
3. **HampelFilterStrategy**: Se eliminan picos anómalos o ruido en mediciones puntuales como la velocidad del viento.
4. **MissingValueImputerStrategy**: Los valores descartados por las estrategias anteriores y los datos faltantes originales se interpolan de manera temporal y continua.
5. **Corrección de Limites**: Acota (clipping) la irradiancia medida al límite físico máximo (constante solar de ~1361 W/m²).
